...
class PlanetarioSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for event in response.css('span.field-content'):
            ...
            time = event.css('div.hora_evento span.date-display-single::text').extract_first()
            date = getDate(event)
            message = scrapy.Request("http://www.planetariodebogota.gov.co" + link,
                                     callback=self.parse_event,
                                     meta={'link': link, 'time': time, 'date': date})
            yield message

    def parse_event(self, response):
        title = response.xpath("//div[@id='titulo-evento']/text()").extract_first()
        place = response.xpath("//div[@id='lugar-evento']/text()").extract_first()
        description = response.xpath("//div[@id='cuerpo-evento']/p/text()").extract_first()
...
def getDate(event):
    date = event.css('div.dia_evento span.date-display-single::text').extract_first()
    if date is None:
        date = event.css('div.dia_evento span.date-display-start::text').extract_first()
    day = str(date).split()
